...
StringJoin[
prefix,
StringReplace[If[StringLength[prefix]==0,Decapitalize[text],Capitalize[text,"\<AllWords\>"]],Whitespace->"\<\>"],
suffix
]
]
];
...
(*debugPrint[32,True]*)(* smoke-test call left over from development; disabled to avoid printing on every load *)
...
(* Demo block left over from development: referenced an undefined symbol ("cat", likely a
   typo) and leaked ape/nut/mouse as global symbols on every notebook load. Disabled. *)
(*
If[$Notebooks,
ape=20;
nut:=20 ape;
...
(* Demo block left over from development: "eee" is never defined. Disabled. *)
(*
Block[{x,y,z},
If[$Notebooks,
